    ...
    Route::domain($hostname->fqdn)->group(function () {
        ...
        Route::middleware(['auth', 'redirect.module', 'locked.tenant'])->group(function () {
            ...
            Route::prefix('restaurants')->group(function () {
                Route::get('/', [RestaurantController::class, 'index'])->name('restaurants.list'); // Listar restaurantes
                Route::get('/create', [RestaurantController::class, 'create'])->name('restaurants.create'); // Mostrar formulario de creación
                Route::post('/', [RestaurantController::class, 'store'])->name('restaurants.store'); // Guardar restaurante
                Route::delete('/{id}', [RestaurantController::class, 'destroy'])->name('restaurants.destroy'); // Eliminar restaurante
            });

            // Gestión de mesas
            Route::get('restaurants/tables/create', 'RestaurantController@showCreateTableForm')->name('tables.showCreateForm'); // Formulario para crear mesa
            Route::post('restaurants/tables', 'RestaurantController@storeTable')->name('tables.create'); // Guardar mesa
            Route::put('restaurants/tables/{id}', 'RestaurantController@updateTable')->name('tables.update'); // Actualizar mesa
            Route::get('restaurants/tables-list', 'RestaurantController@listTables')->name('tables.list'); // Listar mesas de un restaurante
            Route::delete('restaurants/tables/{id}', 'RestaurantController@deleteTable')->name('tables.delete'); // Eliminar mesa

           // Gestión de menús
            Route::get('menus', 'RestaurantController@indexMenus')->name('menus.index');
            Route::get('menus/create', 'RestaurantController@showCreateMenuForm')->name('menus.create');
            Route::post('menus', 'RestaurantController@createMenu')->name('menus.store');
            Route::get('menus/{id}/edit', 'RestaurantController@editMenu')->name('menus.edit');
            Route::put('menus/{id}', 'RestaurantController@updateMenu')->name('menus.update');
            Route::delete('menus/{id}', 'RestaurantController@destroyMenu')->name('menus.destroy');


           // Gestión de pedidos
            Route::prefix('restaurants')->group(function () {
                Route::get('orders-chefs', 'RestaurantController@indexChefsOrders')->name('orders.chefs'); // Mostrar lista de pedidos
                Route::get('orders-waiters', 'RestaurantController@indexWaitersOrders')->name('orders.waiters'); // Mostrar lista de pedidos
                Route::get('orders/create', 'RestaurantController@createOrderView')->name('orders.createView'); // Formulario para crear pedido
                Route::post('orders', 'RestaurantController@createOrder')->name('orders.create'); // Crear pedido
                Route::put('orders/{id}/status', 'RestaurantController@updateOrderStatus')->name('orders.updateStatus'); // Actualizar estado de pedido
                Route::get('tables/{tableId}/orders', 'RestaurantController@listOrders')->name('orders.list'); // Listar pedidos de una mesa
                Route::delete('orders/{id}', 'RestaurantController@deleteOrder')->name('orders.delete'); // Eliminar pedido
                
                // Ruta para listar pedidos listos
                Route::get('orders/ready', 'RestaurantController@readyOrders')->name('orders.ready');
                // Ruta para actualizar el estado del pedido a entregado
